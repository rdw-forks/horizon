...
{
	return (int)((100.0f - target()->status()->soft_def()->total() / (target()->status()->soft_def()->total() + 400.0f) * 90.0f) / 100.0f * damage - target()->status()->hard_def()->total());
}

combat_retaliate_type Combat::weapon_attack()
{
	combat_damage dmg = calculate_weapon_attack();
	
	int64_t damage_total = dmg.left_damage + dmg.right_damage;
...
combat_damage Combat::calculate_weapon_attack()
{
	...
	if (dmg.left_damage)
		dmg.left_damage = calculate_weapon_defense(dmg.left_damage);
	
	dmg.right_damage = calculate_weapon_defense(dmg.right_damage);
	...
	if (dmg.is_critical) {
		hit = true;
		dmg.left_damage += dmg.left_damage * 40 / 100;
		dmg.right_damage += dmg.right_damage * 40 / 100;
	}
...
{
	return (int)((100.0f - target()->status()->hard_mdef()->total() / (target()->status()->hard_mdef()->total() + 100.0f) * 90.0f) / 100.0f * damage - target()->status()->soft_mdef()->total()); 
}
...
combat_damage Combat::calculate_magic_attack()
{
	...
	if (unit()->type() == UNIT_PLAYER) {
		EquipmentListType const& equipments = unit()->downcast<Horizon::Zone::Units::Player>()->inventory()->equipments();
		std::shared_ptr<const item_entry_data> weapon_right = nullptr;
	}

	int magic_atk = unit()->status()->equip_matk()->get_min() + std::rand() % (unit()->status()->equip_matk()->get_max() - unit()->status()->equip_matk()->get_min());

	dmg.right_damage = magic_atk;

	dmg.right_damage -= calculate_magic_defense(dmg.right_damage);
